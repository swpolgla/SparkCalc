...
struct SyntaxHighlighterTests {
    ...
    private func makeHighlighter(text: String) -> (SyntaxHighlighter, NSTextStorage, NSTextView) {
        ...
        return (highlighter, textStorage, textView)
    }
    ...
    @Test func emojiLineUsesUTF16OffsetsForEqualsSign() {
        ...
        let (highlighter, textStorage, _) = makeHighlighter(text: "😀 = 5")

        // The = sign must be at UTF-16 location 3, not grapheme location 2.
        let equalsColor = foregroundColor(at: 3, in: textStorage)
        #expect(equalsColor?.isEqual(to: highlighter.theme.operatorColor) == true)

        // Location 2 is the space before = — should NOT have operator color.
        let spaceColor = foregroundColor(at: 2, in: textStorage)
        #expect(spaceColor?.isEqual(to: highlighter.theme.operatorColor) != true)
    }
    ...
    @Test func emojiLineUsesUTF16OffsetsForNumber() {
        ...
        let numberColor = foregroundColor(at: 5, in: textStorage)
        #expect(numberColor?.isEqual(to: highlighter.theme.number) == true)
    }
    ...
    @Test func cjkLineUsesUTF16OffsetsForEqualsSign() {
        ...
        let (highlighter, textStorage, _) = makeHighlighter(text: "你 = 3")

        // 你 at 0, space at 1, = at 2, space at 3, 3 at 4
        let equalsColor = foregroundColor(at: 2, in: textStorage)
        #expect(equalsColor?.isEqual(to: highlighter.theme.operatorColor) == true)
    }
    ...
    @Test func variableDeclarationGetsDeclarationColor() {
        ...
        let (highlighter, textStorage, _) = makeHighlighter(text: "a = 5")

        let aVarColor = foregroundColor(at: 0, in: textStorage)
        #expect(aVarColor?.isEqual(to: highlighter.theme.variableDeclaration) == true)

        let equalsColor = foregroundColor(at: 2, in: textStorage)
        #expect(equalsColor?.isEqual(to: highlighter.theme.operatorColor) == true)

        let fiveColor = foregroundColor(at: 4, in: textStorage)
        #expect(fiveColor?.isEqual(to: highlighter.theme.number) == true)
    }
    ...
    @Test func knownVariableUseGetsVariableUseColor() {
        ...
        let aUseColor = foregroundColor(at: 6, in: textStorage)
        #expect(aUseColor?.isEqual(to: highlighter.theme.variableUse) == true)
    }

    @Test func functionCallGetsFunctionCallColor() {
        // "sqrt(16)" — 'sqrt' should get functionCall, '16' gets number
        let (highlighter, textStorage, _) = makeHighlighter(text: "sqrt(16)")

        // s=0, q=1, r=2, t=3, (=4, 1=5, 6=6, )=7
        let sqrtColor = foregroundColor(at: 0, in: textStorage)
        #expect(sqrtColor?.isEqual(to: highlighter.theme.functionCall) == true)

        let oneColor = foregroundColor(at: 5, in: textStorage)
        #expect(oneColor?.isEqual(to: highlighter.theme.number) == true)
    }

    @Test func builtInConstantGetsVariableUseColor() {
        // "pi" — built-in constant should get variableUse color
        let (highlighter, textStorage, _) = makeHighlighter(text: "pi")

        let piColor = foregroundColor(at: 0, in: textStorage)
        #expect(piColor?.isEqual(to: highlighter.theme.variableUse) == true)
    }

    // MARK: - Function Body Highlighting

    @Test func functionHeaderNameGetsDeclarationColor() {
        let (highlighter, textStorage, _) = makeHighlighter(text: "f(a, b) {\n  return a + b\n}\nf(1, 2)")

        // Line 1: "f(a, b) {" — 'f' at location 0 gets functionDecl
        let fDeclColor = foregroundColor(at: 0, in: textStorage)
        #expect(fDeclColor?.isEqual(to: highlighter.theme.functionDecl) == true)
    }
    ...
    @Test func functionParameterInHeaderGetsParamDeclarationColor() {
        ...
        let aParamColor = foregroundColor(at: 2, in: textStorage)
        #expect(aParamColor?.isEqual(to: highlighter.theme.localParamDeclaration) == true)
    }
    ...
    @Test func functionParameterInBodyGetsParamUseColor() {
        ...
        let aUseColor = foregroundColor(at: 19, in: textStorage)
        #expect(aUseColor?.isEqual(to: highlighter.theme.localParamUse) == true)
    }

    @Test func functionCallInBodyGetsFunctionCallColor() {
        let (highlighter, textStorage, _) = makeHighlighter(text: "f(x) {\n  return sqrt(x)\n}\nf(16)")

        // Line 2: "  return sqrt(x)"
        // Line 1 "f(x) {" = 6 chars + \n = 7, so line 2 starts at offset 7
        // "  return " = 9 chars, "sqrt" starts at offset 7 + 9 = 16
        let sqrtColor = foregroundColor(at: 16, in: textStorage)
        #expect(sqrtColor?.isEqual(to: highlighter.theme.functionCall) == true)
    }
    ...
    @Test func operatorsGetOperatorColor() {
        let (highlighter, textStorage, _) = makeHighlighter(text: "1 + 2")

        // 1=0, space=1, +=2, space=3, 2=4
        let plusColor = foregroundColor(at: 2, in: textStorage)
        #expect(plusColor?.isEqual(to: highlighter.theme.operatorColor) == true)

        let oneColor = foregroundColor(at: 0, in: textStorage)
        #expect(oneColor?.isEqual(to: highlighter.theme.number) == true)

        let twoColor = foregroundColor(at: 4, in: textStorage)
        #expect(twoColor?.isEqual(to: highlighter.theme.number) == true)
    }
